...
def test_update_schedule_for_user(
    ...
    response = client.put(
        f"{settings.API_V1_STR}/schedule_user/update_schedule_for_user/"
        f"{schedule.user_id}/schedule_id/{schedule.schedule_id}",
        headers=superuser_token_headers,
...
def test_delete_schedule_user(
    ...
    response = client.delete(
        f"{settings.API_V1_STR}/schedule_user/delete_schedule_user/user_id/"
        f"{schedule.user_id}/schedule_id/{schedule.schedule_id}",
        headers=superuser_token_headers
    )
    assert response.status_code == 200
    ...
    for schedule_user in content:
        assert schedule_user["user_id"] == schedule.user_id
        assert schedule_user["schedule_id"] == schedule.schedule_id
        assert schedule_user["shift_start"] == schedule.shift_start \
            .strftime("%Y-%m-%dT%H:%M:%S")
        assert schedule_user["shift_end"] == schedule.shift_end \
            .strftime("%Y-%m-%dT%H:%M:%S")
