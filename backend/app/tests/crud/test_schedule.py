...
from app.tests.utils.schedule import get_last_day, random_bool, \
...
def test_create_schedule(db: Session):
    ...
    day_off = random_bool()
    schedule_in_db = crud.schedule.create(db,
                                          start_day=start_day,
                                          day_off=day_off)
...
def test_update_schedule(db: Session):
    ...
    schedule_updated_in_db = crud.schedule \
        .update(db, db_obj=schedule_new, obj_in=schedule_updated)
